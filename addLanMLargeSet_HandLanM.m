...
%landmarks
 ...
 %load('handPickedResult/optWeight_narrowerDP.mat');
...
iteration = 90;
    ...
    maxAccuracy = 0.0;
    numEle = size(trainFreeman,1)/10;
    numLanM = size(lanFreeman,1)/10;
    ...
    for m = 1:iteration,
        ...
        for k = 1:size(information,1)
            ...
            index_exam = current.index_example;    
            editDistance_examInc = EditDistanceWeight(trainFreeman{index_exam,1},lanFreeman{index_inc_lanM(1),1},weightMatrix,threshold);
            editDistance_examCor = EditDistanceWeight(trainFreeman{index_exam,1},lanFreeman{index_cor_lanM(1),1},weightMatrix,threshold);
        ...
        if(index_newLandmark>0)
            ...
            worstExample(m,:) = [testData(index_newLandmark,:),current.predicted_label];
            newDistanceArray = zeros(size(trainFreeman,1),1);
            for i = 1:size(trainFreeman,1),
                d = EditDistanceWeight(trainFreeman{i,1},addLandmark,weightMatrix,threshold);        
                %d = exp(-d^2/sigma);
