...
load('sampleddata2/trainData.mat');
load('sampleddata2/trainIndex.mat');
load('sampleddata2/lanMatrix.mat');
load('sampleddata2/lanIndex.mat');
load('sampleddata2/backupLanMatrix.mat');
load('sampleddata2/backupLanIndex.mat');
load('sampleddata2/newMatrix.mat');
load('sampledData2/allLanIndex.mat');
...
for m = 1:iteration,
    ...
    %adjust the weight automatically for each example independently 
    %{
    for k = 1:size(information,1),
        ...
        incorrSequence = findPathWeight(trainFreeman{index_exam,1},lanFreeman{index_inc_lanM(1),1},tempWeigtMatrix);
        ...
        for i = 1:size(incorrSequence,1),
            ...
            if(incorrSequence(i,3)==0),
                ...
                tempWeigtMatrix(row,column) = tempWeigtMatrix(row,column) + 1.0;
                z = z + 1;
            end
        end
                
    end
    weightMatrix = tempWeigtMatrix;
    %}
