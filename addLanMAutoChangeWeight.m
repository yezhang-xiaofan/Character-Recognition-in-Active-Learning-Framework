%add landmarks and adjust the weight matrix automatically 
...
worstMatrix = zeros(iteration,numPixel*numPixel+2);
for m = 1:iteration,   
    %L1-Loss SVM 
    %compute kernel matrix
    for i = 1:size(trainFreeman,1),
    ...
    if(index_newLandmark>0)
        ...
        worstMatrix(m,:) = [testData(index_newLandmark,:),current.predicted_label];
        %{
    ...
    tempStr = strcat(num2str(m),'th ',' adding Landmark');
    disp(tempStr);
    
    %change weight automatically
    for q = 1:15,
        for i = 1:size(trainFreeman,1),
            for j = 1:size(lanFreeman,1),
                d = EditDistanceWeight(trainFreeman{i,1},lanFreeman{j,1},weightMatrix);
    ...
    weightMatrix = optimalWeight;
    accuracyArray(m) = maxAccuracy;
    tempStr = strcat(num2str(m),' th changing weight');
    disp (tempStr);
end
disp('finish');
